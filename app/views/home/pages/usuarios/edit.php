...
<form id="cadastro" method="POST" enctype="multipart/form-data">
    <div class="row g-2">
        ...
        <div class="col-md-3">
            <div class="form-floating">
                <?php if ($_SESSION['user_function_id'] <= 2) { ?>
                    <select class="form-select" id="user_function_id" name="user_function_id" aria-label="">
                    <?php } else { ?>
                        <select class="form-select" id="user_function_id" name="user_function_id" aria-label="" disabled>
                        <?php } ?>
                        <option disabled <?= empty($result['user_function_id']) ? 'selected' : ''; ?>>Selecione função</option>
                        <?php
                        $functions = $read->fetchAll("SELECT * FROM user_function ORDER BY user_function_id ASC");
                        foreach ($functions as $funcao):
                            $selected = ($funcao['user_function_id'] == $result['user_function_id']) ? 'selected' : '';
                        ?>
        ...
        <div class="col-md-3">
            <div class="form-floating">
                <input type="text" class="form-control" id="user_email" name="user_email" placeholder="Email" value="<?php echo isset($result['user_email']) ? $result['user_email'] : ''; ?>">
        ...
        <div class="col-md-3">
            <div class="form-floating">
                <select class="form-select" id="setor_id" name="user_setor_id">
                    <option disabled selected value="">Carregando...</option>
                </select>
        ...
        <div class="form-group col-md-3">
            <div class="form-floating">
                <input type="password" name="user_pass" class="form-control" id="user_pass" value="<?php if (!empty($ClienteData['user_pass'])) echo $ClienteData['user_password']; ?>">
