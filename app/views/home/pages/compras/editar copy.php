        ...
        <table class="table table-striped">
            ...
            <tbody>
                    <tr class="table-info">
                        ...
                        <td data-label="Pagamento" style="width:10%" class="text-left">Pagamento</td>
                        <td data-label="Comprado por" style="width:10%" class="text-left">Comprado por</td>
                        <td data-label="Data Compra" style="width:10%" class="text-left">Data Compra</td>
                        <td data-label="Prev Entrega" style="width:10%" class="text-left">Prev Entrega</td>
...
<div class="table-responsive mt-3">
  <table class="table table-striped">
    <thead>
      <tr class="table-dark">
        ...
        <td data-label="Pagamento" style="width:10%" class="text-left">Pagamento</td>
        <td data-label="Comprado por" style="width:10%" class="text-left">Comprado por</td>
        <td data-label="Data da compra" style="width:10%" class="text-left">Data Compra</td>
        <td data-label="Prev. Entrega" style="width:10%" class="text-left">Prev Entrega</td>
        <td data-label="Editar" style="width:5%" class="text-center">Editar</td>
      </tr>
    </thead>
    <tbody>
      <tr class="table-info">
        ...
        <td style="width:100px" class="text-left"><?= $compra['pagamento_id'] ?? ' ' ?></td>
        ...
        <td class="text-left"><?= date('d/m/Y', strtotime($compra['cad_data'])); ?> </td>
        ...
        <?php if ($data1 < $data2) { ?>
          <td style="width:100px; font-family:Trebuchet MS,Helvetica,sans-serif; color:green;" class="text-left"><?= date('d/m/Y', strtotime($compra['previsao'])); ?> </td>
        <?php } elseif ($data1 > $data2) { ?>
          <td style="width:100px; font-family:Trebuchet MS,Helvetica,sans-serif; color:red;" class="text-left"><?= date('d/m/Y', strtotime($compra['previsao'])); ?> </td>
        <?php } elseif ($data1 == $data2) { ?>
          <td style="width:100px; font-family:Trebuchet MS,Helvetica,sans-serif; color:blue;" class="text-left"><?= date('d/m/Y', strtotime($compra['previsao'])); ?> </td>
        <?php } ?>
        <td class="text-center">
          <a href="painel.php?exe=compras/editar_dados&compraId=<?= $compraId ?>" title="Editar Dados"><i class="fa fa-pencil mr-1 fa-fw icon" title="Editar Dados"></i></a>
        </td>
      </tr>
  </table>
</div>
  ...
  <form method="POST" action="">
    ...
    <!--                                                                                   
                    <div class="calculo">
                        <div class="result_soma">Total:</div>
                        <div id="totalCompra" class="soma"></div>
                    </div>
                                                                                                    -->


    <input type="hidden" class="form-control" name="cad_autor" value="<?= $compra['cad_autor']; ?>" />


    <input type="submit" name="submitBtn" id="submitBtn" value="Confirmar" class="btn btn-success" />
  </form>
  <br />
    ...
    <tbody>
      ...
      ?>
        <tr>
          ...
          <td data-label="Ações">
            <a href="painel.php?exe=compras/editar_produtos&compraId=<?= $dados['id'] ?>" title="Editar Produto"><i class="fa fa-pencil mr-1 fa-fw icon" title="Editar Produto"></i></a>
            <a href="#" data-id='<?php echo $dados['id'] . '-' . $dados['produto_id'] . '-' . $dados['compra_id'] ?>' class="excluirproduto link" title="Excluir"><i class="fas fa-trash mr-1 icon" title="Excluir"></i></a>
          </td>
